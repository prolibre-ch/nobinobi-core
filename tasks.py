...
def coverage(c):
    ...
    """
    c.run("coverage run --source nobinobi-core runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/nobinobi-core.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ nobinobi_core")
...
def lint(c):
    ...
    """
    c.run("flake8 nobinobi-core tests")
...
def release(c, bumpsize=''):
    ...
    import nobinobi_core
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=nobinobi_core.__version__))
    c.run("git push --tags")
